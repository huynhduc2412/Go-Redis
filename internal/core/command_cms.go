...
func cmdCMSINITBYDIM(args []string) []byte {
	...
	if exist {
		return Encode(errors.New("CMS: key already existed") , false)
	}
	cmsStore[key] = data_structure.CreateCMS(uint32(width) , uint32(depth))
	return constant.RespOk
}

//based on math formula and probability 
//error = 1% , prob = 1% means 99% counter error rate <= 1%
...
func cmdCMSINCRBY(args []string) []byte {
	...
	if !exist {
		return Encode(errors.New("CMS: key doesn't exist") , false)
	}
	var res []string
